...
describe("SessionStorageAdapter", () => {
  describe("_constructor", () => {
    it("should throw if sessionStorage is not available", () => {
      delete global.sessionStorage;
      expect(() => new SessionStorageAdapter({ dbName: "test" })).toThrow(
    ...
    it("should start expire watcher", () => {
      vi.useFakeTimers();
      const spy = vi.spyOn(SessionStorageAdapter.prototype, "clearExpire");

      new SessionStorageAdapter({ dbName: "testDB" });
    ...
    it("should use default version and description if not provided", () => {
      const adapter = new SessionStorageAdapter({ dbName: "defaultsDB" });

      const meta = JSON.parse(global.sessionStorage.getItem(adapter.metaKey));
    ...
    it("should correctly store dbName and derived keys", () => {
      const adapter = new SessionStorageAdapter({ dbName: "derivedDB" });
    ...
    it("should create metadata only once even if instantiated twice with same dbName", async () => {
      const adapter1 = new SessionStorageAdapter({ dbName: "metaOnce" });
      const meta1 = global.sessionStorage[adapter1.metaKey];

      const adapter2 = new SessionStorageAdapter({ dbName: "metaOnce" });
      const meta2 = global.sessionStorage[adapter2.metaKey];
    ...
    it("should call _initMeta and _startExpireWatcher", () => {
      const metaSpy = vi.spyOn(SessionStorageAdapter.prototype, "_initMeta");
      const watcherSpy = vi.spyOn(
        SessionStorageAdapter.prototype,
        "_startExpireWatcher"
      );

      new SessionStorageAdapter({ dbName: "callSpyDB" });
  ...
  describe("set/get", () => {
    ...
    it("should expire key after given time", async () => {
      ...
      await adapter.set("tempKey", "tempValue", 800);

      // Após 500ms, ainda deve existir
      await sleep(500);
  ...
  describe("delete", () => {
    it("should remove an existing item from sessionStorage", async () => {
      await adapter.set("toDelete", { remove: true });

      expect(
        sessionStorage.getItem(adapter._fullKey("toDelete"))
      ).not.toBeNull();

      await adapter.delete("toDelete");

      expect(sessionStorage.getItem(adapter._fullKey("toDelete"))).toBeNull();
    });
    ...
    it("should remove expiration metadata when deleting a key", async () => {
      ...
      let exp = JSON.parse(sessionStorage.getItem(adapter.expireKey));
      expect(exp).toHaveProperty("withExpire");

      await adapter.delete("withExpire");

      exp = JSON.parse(sessionStorage.getItem(adapter.expireKey));
    ...
    it("should not alter expiration data of other keys", async () => {
      ...
      const exp = JSON.parse(sessionStorage.getItem(adapter.expireKey));
  ...
  describe("has", () => {
    it("should return true if the key exists in sessionStorage", async () => {
    ...
    it("should return false when the key is expired", async () => {
      await adapter.set("willExpire", "x", 100); // expires in 100ms
  ...
  describe("clear", () => {
    ...
    beforeEach(() => {
      adapter = new SessionStorageAdapter({ dbName: "testDB" });
    });
    ...
    it("should clear all data, meta and expire keys", async () => {
      ...
      expect(sessionStorage.getItem(adapter.metaKey)).toBeNull();
      expect(sessionStorage.getItem(adapter.expireKey)).toBeNull();
    });

    it("should not remove unrelated keys", async () => {
      sessionStorage.setItem("unrelated_key", "preserve this");

      await adapter.set("somekey", "value");
      await adapter.clear();

      expect(sessionStorage.getItem("unrelated_key")).toBe("preserve this");
    });
  });
  ...
  describe("reset", () => {
    ...
    it("should not throw error if sessionStorage is empty", async () => {
      await adapter.reset();
      const list = await adapter.list();
      expect(list).toEqual([]);
    });
